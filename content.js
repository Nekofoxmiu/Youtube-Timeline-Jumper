...
(async () => {
    ...
    try {
        ...
        getCurrentVideoIdModule = await import('./lib/getCurrentVideoId.js');
    } catch (error) {
    ...
    const { getCurrentVideoId } = getCurrentVideoIdModule;
        ...
        function handleClick(event) {
            ...
            if (editableElement) {
                ...
                // 啟用編輯模式
                enableEditMode(editableElement);
            }
        }
         ...
         */
        function enableEditMode(editableElement) {
            ...
            // 設置失去焦點事件處理
            editableElement.addEventListener('blur', () => handleBlur(editableElement, originalAttr), { once: true });
         ...
         */
        function handleBlur(editableElement, originalAttr) {
            if (editableElement.tagName === 'INPUT') {
