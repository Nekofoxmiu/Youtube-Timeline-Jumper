...
export function applySegmentFilterPredictions(segments, predictions, options = {}) {
  ...
  for (let sortedIndex = 0; sortedIndex < sorted.length; sortedIndex += 1) {
    ...
    const startTrimDeltaSec = clamp(prediction.startTrimDeltaSec ?? prediction.start_delta_sec ?? 0, -opts.trimClampSec, opts.trimClampSec) * opts.trimScale;
    const endTrimDeltaSec = clamp(prediction.endTrimDeltaSec ?? prediction.end_delta_sec ?? 0, -opts.trimClampSec, opts.trimClampSec) * opts.trimScale;
...
export async function loadEdgeTrimAdvisorModel({ ort: ortRuntime = null, modelUrl = null, metaUrl = null, executionProviders = ['wasm'] } = {}) {
  const ort = resolveOrt(ortRuntime);
  if (!ort?.InferenceSession || !ort?.Tensor) {
    throw new Error('ONNX Runtime Web is unavailable for edge trim advisor.');
  }
  const resolvedModelUrl = modelUrl || (globalThis.chrome?.runtime?.getURL
    ? chrome.runtime.getURL('models/fireredvad/aed/edge_trim_advisor.onnx')
    : 'models/fireredvad/aed/edge_trim_advisor.onnx');
  const resolvedMetaUrl = metaUrl || (globalThis.chrome?.runtime?.getURL
    ? chrome.runtime.getURL('models/fireredvad/aed/edge_trim_advisor.meta.json')
