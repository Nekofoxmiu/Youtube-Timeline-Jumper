...
const OFFLINE_FINAL_SMOOTHING_OVERRIDES = Object.freeze({
  ...
  // Offline has full context, but the last aggressive edge/music-only override
  // regressed the full batch gate. Keep the validated baseline until a per-profile
  // tuning run proves a safer replacement.
...
const LIVE_REALTIME_AED60_SMOOTHING_OVERRIDES = Object.freeze({
  // AED60 is latency-limited and already has known per-song failures. Keep the
  // smoothing thresholds pinned to the validated baseline; its separation is in
  // the finalizer profile, where PCM30-only start-boundary logic stays disabled.
  tracker: Object.freeze({}),
  modelRun: Object.freeze({}),
  decision: Object.freeze({}),
});
