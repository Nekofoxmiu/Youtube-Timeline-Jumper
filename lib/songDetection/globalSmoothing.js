...
function summarizeMusicOnlySpan(frames, startSec, endSec, candidateCount) {
  const stats = buildMusicOnlySpanStats(
    ...
    endSec,
    candidateCount,
    MODEL_RUN_SEGMENT_RULES.musicOnlyExcludeMinDurationSec
  );
  const pass = musicOnlyStatsPass(stats, {
    minCandidateRatio: MODEL_RUN_SEGMENT_RULES.musicOnlyMinCandidateRatio,
    minMusicMean: MODEL_RUN_SEGMENT_RULES.musicOnlyMinMusicMean,
    minMusicP80: MODEL_RUN_SEGMENT_RULES.musicOnlyMinMusicP80,
    maxSingingMean: MODEL_RUN_SEGMENT_RULES.musicOnlyMaxSingingMean,
    maxSingingP90: MODEL_RUN_SEGMENT_RULES.musicOnlyMaxSingingP90,
    maxSingingRatioMean: MODEL_RUN_SEGMENT_RULES.musicOnlyMaxSingingRatioMean,
    maxSpeechMean: MODEL_RUN_SEGMENT_RULES.musicOnlyMaxSpeechMean,
    maxSpeechP90: MODEL_RUN_SEGMENT_RULES.musicOnlyMaxSpeechP90,
    maxSpeechRatioMean: MODEL_RUN_SEGMENT_RULES.musicOnlyMaxSpeechRatioMean,
  });
  return pass ? formatMusicOnlyExclusionSpan(stats, 'long-music-only-low-vocal') : null;
}
...
function getClusterMusicOnlyThresholds() {
  return {
    minCandidateRatio: MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMinCandidateRatio,
    minMusicMean: MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMinMusicMean,
    minMusicP80: MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMinMusicP80,
    maxSingingMean: MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMaxSingingMean,
    maxSingingP90: MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMaxSingingP90,
    maxSingingRatioMean: MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMaxSingingRatioMean,
    maxSpeechMean: MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMaxSpeechMean,
    maxSpeechP90: MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMaxSpeechP90,
    maxSpeechRatioMean: MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMaxSpeechRatioMean,
  };
}
...
function summarizeSegmentMusicOnlyCandidate(analyses, segment) {
  ...
  if (durationSec < MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMinSegmentDurationSec) return null;
...
function summarizeClusterMusicOnlySpan(analyses, cluster) {
  ...
  const enoughDuration = totalSegmentDurationSec >= MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMinTotalDurationSec
    || (
      spanDurationSec >= MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMinSpanDurationSec
      && totalSegmentDurationSec >= MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMinTotalDurationSec * 0.82
    );
...
function findClusteredMusicOnlySpans(segments, analyses) {
  ...
  for (const candidate of candidates) {
    ...
    if (gapSec <= MODEL_RUN_SEGMENT_RULES.musicOnlyClusterMaxGapSec) {
...
function segmentLooksLikeWeakHeadMusicOnly(segment, matchedSpan, analyses, overlapSec) {
  const normalizedSegment = normalizeOutputSegment(segment);
  const durationSec = segmentDuration(normalizedSegment);
  if (durationSec <= 0 || durationSec > MODEL_RUN_SEGMENT_RULES.musicOnlyWeakHeadDropMaxDurationSec) {
    return false;
  }
  if (overlapSec < MODEL_RUN_SEGMENT_RULES.musicOnlyWeakHeadDropMinOverlapSec) {
    return false;
  }
  const startsInsideOrNearSpan = normalizedSegment.startSec <= (
    Number(matchedSpan.endSec) + MODEL_RUN_SEGMENT_RULES.musicOnlyWeakHeadDropMaxStartAfterSpanSec
  );
  if (!startsInsideOrNearSpan) return false;

  const support = summarizeSegmentModelSupport(analyses, normalizedSegment);
  return support.frameCount >= MODEL_RUN_SEGMENT_RULES.unsupportedTrackerMinFrames
    && support.modelAboveRatio <= MODEL_RUN_SEGMENT_RULES.musicOnlyWeakHeadDropMaxModelRatio
    && support.musicMean >= MODEL_RUN_SEGMENT_RULES.musicOnlyWeakHeadDropMinMusicMean
    && support.singingMedian <= MODEL_RUN_SEGMENT_RULES.musicOnlyWeakHeadDropMaxSingingMedian
    && support.singingMean <= MODEL_RUN_SEGMENT_RULES.musicOnlyWeakHeadDropMaxSingingMean;
}
  ...
  for (const segment of segments) {
    const matchedSpan = excludedMusicOnlySpans.find((span) => {
      ...
      return overlapRatio >= MODEL_RUN_SEGMENT_RULES.musicOnlyDropMinOverlapRatio
        ...
        || segmentLooksLikeWeakHeadMusicOnly(segment, span, analyses, overlapSec);
    });
    if (matchedSpan) {
