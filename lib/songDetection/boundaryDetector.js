...
function normalizeFrame(frame) {
  ...
  return {
    ...
    songProbability: clamp(toFiniteNumber(normalized.temporalHeadProbability ?? normalized.songProbability, 0), 0, 1),
    singing: clamp(toFiniteNumber(normalized.singingProbability ?? normalized.singingMean, 0), 0, 1),
    music: clamp(toFiniteNumber(normalized.musicProbability ?? normalized.musicMean, 0), 0, 1),
    speech: clamp(toFiniteNumber(normalized.speechProbability ?? normalized.speechMean, 0), 0, 1),
    audioRms: Math.max(0, toFiniteNumber(normalized.audioRms, 0)),
    audioPeak: Math.max(0, toFiniteNumber(normalized.audioPeak, 0)),
    spectralCentroid: clamp(toFiniteNumber(normalized.spectralCentroid, 0), 0, 1),
    spectralFlatness: clamp(toFiniteNumber(normalized.spectralFlatness, 0), 0, 1),
    spectralFlux: clamp(toFiniteNumber(normalized.spectralFlux, 0), 0, 1),
    midEnergyRatio: clamp(toFiniteNumber(normalized.midEnergyRatio, 0), 0, 1),
    highEnergyRatio: clamp(toFiniteNumber(normalized.highEnergyRatio, 0), 0, 1),
  };
}
