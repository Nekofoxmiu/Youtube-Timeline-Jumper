...
function buildOfflineAnalysisFrames(stats, temporalHeadResult, runtime, timeOffsetSec, {
  ...
  for (let index = 0; index < stats.timeSec.length; index += 1) {
    ...
    const frame = normalizeAnalysisFrame({
      ready: true,
...
export class FireRedAedSongDetector {
  constructor({
    ...
    chunkSec = 60,
    overlapSec = 120,
    liveAnalysisMethod = 'aed-cache-60s',
  }) {
    ...
    this.liveAnalysisMethod = String(liveAnalysisMethod || 'aed-cache-60s');
  ...
  async initialize() {
    ...
    this.frameBuilder = new StreamingFrameBuilder({
      ...
      int16ToFloat32(pcm16),
      {
        ...
        internalChunkSec: 60,
        sourceMode: this.liveAnalysisMethod === 'pcm-rollover-30min' ? 'live-pcm30' : 'live-realtime-aed60',
        sourceRangeId: options.sourceRangeId ?? null,
      }
    ),
    });
  }
  ...
  setPlaybackRate(value) {
    if (this.frameBuilder && typeof this.frameBuilder.setTimeScale === 'function') {
      this.frameBuilder.setTimeScale(value);
    }
  }

  getBufferedPcmSummary() {
    if (!this.frameBuilder || typeof this.frameBuilder.getProgressSummary !== 'function') {
      return {
        bufferedSec: 0,
        retainedSec: 0,
        chunkProgressSec: 0,
        chunkRemainingSec: this.chunkSec,
        totalCapturedSec: 0,
        totalAnalyzedSec: 0,
        bufferedBytes: 0,
        chunkSec: this.chunkSec,
        overlapSec: this.overlapSec,
        playbackRate: 1,
        videoChunkProgressSec: 0,
        videoTotalCapturedSec: 0,
        videoTotalAnalyzedSec: 0,
      };
    }
    const summary = this.frameBuilder.getProgressSummary();
    return {
      ...summary,
      bufferedSec: roundNumber(summary.bufferedSec, 3),
      retainedSec: roundNumber(summary.retainedSec, 3),
      chunkProgressSec: roundNumber(summary.chunkProgressSec, 3),
      chunkRemainingSec: roundNumber(summary.chunkRemainingSec, 3),
      totalCapturedSec: roundNumber(summary.totalCapturedSec, 3),
      totalAnalyzedSec: roundNumber(summary.totalAnalyzedSec, 3),
      chunkSec: this.chunkSec,
      overlapSec: this.overlapSec,
      playbackRate: roundNumber(summary.playbackRate, 3),
      videoChunkProgressSec: roundNumber(summary.videoChunkProgressSec, 3),
      videoTotalCapturedSec: roundNumber(summary.videoTotalCapturedSec, 3),
      videoTotalAnalyzedSec: roundNumber(summary.videoTotalAnalyzedSec, 3),
    };
  }
