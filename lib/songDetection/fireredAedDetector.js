...
async function loadTemporalHeadRuntime(ort) {
  ...
  const sessionResult = await createOrtSessionWithFallback(ort, modelUrl, {
    label: 'FireRed temporal song head',
  });
...
export class FireRedAedSongDetector {
  ...
  async analyze() {
    ...
    const outputs = await runAedSession(this.runtime, feeds);
    ...
    const spectrum = summarizeAudioSpectrum(
      samples16k,
      this.runtime,
      Math.round(TARGET_SAMPLE_RATE * TEMPORAL_HEAD_HOP_SEC),
      this.spectralState.previousSpectrum
    );
    this.spectralState.previousSpectrum = spectrum.normalizedSpectrum;
    ...
    return {
      ...
      lowEnergyRatio: spectrum.lowEnergyRatio,
      midEnergyRatio: spectrum.midEnergyRatio,
      highEnergyRatio: spectrum.highEnergyRatio,
