...
export function getCurrentVideoTime() {
    ...
    return TimeSlot.fromTotalseconds(video.currentTime);
}

export function getRoundedCurrentVideoTime() {
    const video = document.querySelector('video');
    if (!video) return null;
    return TimeSlot.fromTotalseconds(Math.round(video.currentTime));
}
