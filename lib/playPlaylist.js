...
async function stopCurrentPlayback(tabId, { resetId = true } = {}) {
    ...
    document.querySelectorAll('.ytj-playing-item').forEach(item => {
        ...
        item.querySelector('.ytj-playing-indicator')?.classList.remove('playing');
        item.querySelector('.ytj-drag-handle')?.classList.remove('playing');
    });

    if (resetId && tabId != null) {
...
async function updateStyles(item, add) {
    ...
    const playingIndicator = item.querySelector('.ytj-playing-indicator');
    item.querySelector('.ytj-drag-handle')?.classList.remove('playing');
    if (add) {
...
export async function playPlaylist(startIndex, endIndex, sendResponse, tabId) {
    ...
    try {
        ...
        // 停止當前播放
        if (isPlaying) {
            await stopCurrentPlayback(tabId, { resetId: false });
        ...
        for (const [index, item] of playlistItems.entries()) {
            ...
            const startTime = parseFloat(item.querySelector('.ytj-playlist-item-text-start').getAttribute('timeat'));
            const endTime = parseFloat(item.querySelector('.ytj-playlist-item-text-end').getAttribute('timeat'));
            ...
            const currentIdAfter = await getStorageValue(`currentPlayId_${tabId}`);
            if (thisPlayId !== currentIdAfter) break;
