...
export async function handleRuntimeMessage(request, sender, sendResponse, context) {
    try {
        ...
        if (request.action === 'removePlaylist') {
            ...
            try {
                await chrome.runtime.sendMessage({ action: 'stopSongDetectionForActiveTab' });
            } catch (error) {
                console.debug('stopSongDetectionForActiveTab failed during removePlaylist:', error);
            }
        ...
        if (request.action === 'initializePlaylist') {
            if (extensionWorkOrNot === false) {
                extensionWorkOrNot = true;
                await chrome.storage.local.set({ extensionWorkOrNot: extensionWorkOrNot }, () => {
                    console.info('Extension state saved:', extensionWorkOrNot);
                });
            }
