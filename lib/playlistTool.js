...
export class PlaylistTimeManager {
    ...
    */
    static getVideoDuration = () => {
        ...
        const videoDuration = TimeSlot.fromTotalseconds(video.duration);
...
export const getandUpdatePlaylistState = (sharedState) => {
    ...
    const nowPlaylistState = sharedState.playlistItems.map(item => {
        ...
        const confidenceRaw = item.dataset?.confidence;
        const provisionalRaw = item.dataset?.provisional;
        const detectorVersion = item.dataset?.detectorVersion || '';

        const output = { start: startTime.toformatObject(), end: endTime.toformatObject(), title };
        if (itemType && itemType !== 'manual') output.type = itemType;
        if (confidenceRaw !== undefined && confidenceRaw !== '') {
            const confidence = Number(confidenceRaw);
            if (Number.isFinite(confidence)) output.confidence = confidence;
        }
        if (provisionalRaw === 'true' || provisionalRaw === 'false') {
            output.provisional = provisionalRaw === 'true';
        }
        if (detectorVersion) output.detectorVersion = detectorVersion;
        return output;
    });
