...
export function handleBlur(editableElement, originalAttr, playlistState, playlistTimeManager) {
    ...
    // mark lastModified on the parent list item when title/time edited
    try {
        const listItem = editableElement.closest('.ytj-playlist-item');
        if (listItem) {
            const nowStr = new Date().toISOString();
            listItem.dataset.lastModified = nowStr;
            try {
                if (playlistTimeManager && playlistTimeManager.stateManager && typeof playlistTimeManager.stateManager.saveMeta === 'function') {
                    (async () => {
                        const existingMeta = await playlistTimeManager.stateManager.loadMeta() || {};
                        const newMeta = { ...(existingMeta || {}), lastModified: nowStr };
                        await playlistTimeManager.stateManager.saveMeta(newMeta);
                    })();
                }
            } catch (e) {}
        }
