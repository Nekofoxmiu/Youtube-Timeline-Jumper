...
if (!args.frames || !args.manual || !args.out) {
  throw new Error('Usage: node tools/simulate_live_smoothing.mjs --frames <frames.json> --manual <manual.txt> --out <summary.json> [--lookahead-sec 20 30 60] [--step-sec 30]');
}
...
const lookaheads = parseNumberList(args['lookahead-sec'], [20, 30, 60]);
...
for (const lookaheadSec of lookaheads) {
  ...
  for (let nowSec = Math.max(30, startSec + stepSec); nowSec <= endSec; nowSec += stepSec) {
    ...
    const smoothing = smoothFireRedAnalyses(visibleFrames, visibleEndSec, { startSec });
    const visibleTimes = frames
      .map((frame) => Number(frame.timeSec) || 0)
      .filter((time) => time > nowSec - stepSec && time <= nowSec);
    const liveSegments = clipSegmentsToTime(smoothing.segments, nowSec);
